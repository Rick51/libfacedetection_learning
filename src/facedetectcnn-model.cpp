...
extern float input_pConvCoefScales[NUM_CONV_LAYER];
#else
...
float param_pMean[3] = { 104.0,117.0,123.0};
float param_pScale[3] = {1.0, 1.0, 1.0};
...
#if defined(_ENABLE_INT8_CONV)
        param_pFilters[i].weight_scale = param_pConvCoefScales[i];
        param_pFilters[i].data_scale = input_pConvCoefScales[i];
#endif
        param_pFilters[i].group = param_pConvInfo[i].group;
...
vector<ObjectRect> objectdetect_cnn(unsigned char * rgbImageData, int width, int height, int step)
{
    ...
    total = 0.0;
    TIME_START;
    //inputImage.setDataFromImage(rgbImageData, width, height, 4, step, param_pMean);
    inputImage.setDataFrom3x3S2P1to1x1S1P0FromImage(rgbImageData, width, height, 4, step, param_pMean ,param_pScale);
...
/***************CONV6*********************/
    ...
    int conv6idx = 15;
    TIME_START;
    normalize(pConvDataBlobs + conv6idx, param_pConv6Norm);
    TIME_END("norm6");
    ...
    for (int i = 0; i < facesInfo.width; i++)
    {
        ...
        ObjectRect r;
        r.score = score;
        r.width = int( ((bbxmax - bbxmin) * width + (bbymax - bbymin) * height + 1) / 2);
        r.height = r.width;
        r.xmin = int(((bbxmin + bbxmax) * width - r.width + 0.5f) / 2);
        r.ymin = int(((bbymin + bbymax) * height - r.height + 0.5f) / 2);
        faces.push_back(r);
    }
...
int * facedetect_cnn(unsigned char * result_buffer, unsigned char * rgb_image_data, int width, int height, int step)
{
    ...
    vector<ObjectRect> faces = objectdetect_cnn(rgb_image_data, width, height, step);
    ...
    for (int i = 0; i < num_faces; i++)
    {
        ...
        p[0] = (short)faces[i].xmin;
        p[1] = (short)faces[i].ymin;
        p[2] = (short)faces[i].width;
        p[3] = (short)faces[i].height;
